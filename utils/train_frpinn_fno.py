...
from losses.frpinn_fno_loss import FRPINNsFNOLoss
from data.data_loader import PhysicsSampler
from data.medium_profile import (
    MultiMediumDataset,
    create_default_medium_library, create_default_fno_config
)
from config_utils import load_config, config_to_frpinn_fno_args
...
class GradientMonitor:
    def __init__(self, log_interval=100, alert_threshold=100.0, nan_alert=True):
    ...
    def plot_gradient_norms(self, save_path=None):
        ...
        ax.set_title('Gradient Norm During Training (FRPINNs-FNO)')
...
class TrainingLogger:
    def __init__(self, log_dir='logs', log_file='training_fno.log'):
    ...
    def log_epoch(self, epoch, total_epochs, loss_dict, elapsed=None, extra_info=None):
...
class EvaluationCallback:
    def __init__(self, eval_interval=1000, save_dir='evaluation_fno'):
    ...
    def evaluate(self, model, dataset, epoch, device='cpu',
                 medium_profiles=None, freq_idx=0):
        ...
        with torch.no_grad():
            if medium_profiles is not None:
                mid_indices = dataset.medium_indices[indices]
                unique_mids = mid_indices.unique()
                pred_parts = []
                for mid in unique_mids:
                    mask = mid_indices == mid
                    x_part = x_eval[mask]
                    key = (int(mid.item()), freq_idx)
                    mp = medium_profiles.get(key, None)
                    if mp is None:
                        for k in medium_profiles:
                            if k[0] == int(mid.item()):
                                mp = medium_profiles[k]
                                break
                    if mp is not None:
                        pred_parts.append(model(x_part, mp))
                    else:
                        pred_parts.append(model.forward_local_only(x_part))
                pred = torch.cat(pred_parts, dim=0)
            else:
                pred = model.forward_local_only(x_eval)
...
class MediumScheduler:
    def __init__(self, n_models, strategy='random', n_per_epoch=5,
                 curriculum_stages=None):
        self.n_models = n_models
        self.strategy = strategy
        self.n_per_epoch = min(n_per_epoch, n_models)
        self.curriculum_stages = curriculum_stages or [0.3, 0.6, 1.0]
        self.current_stage = 0
        self.epoch_count = 0

    def get_medium_indices(self, epoch, total_epochs):
        self.epoch_count = epoch

        if self.strategy == 'random':
            return np.random.choice(
                self.n_models, self.n_per_epoch, replace=False
            )

        elif self.strategy == 'sequential':
            start = (epoch * self.n_per_epoch) % self.n_models
            indices = [(start + i) % self.n_models for i in range(self.n_per_epoch)]
            return np.array(indices)

        elif self.strategy == 'curriculum':
            progress = epoch / max(1, total_epochs)
            stage_thresholds = self.curriculum_stages
            n_available = max(1, int(self.n_models * stage_thresholds[min(
                self.current_stage, len(stage_thresholds) - 1
            )]))
            if (self.current_stage < len(stage_thresholds) - 1 and
                    progress >= stage_thresholds[self.current_stage]):
                self.current_stage += 1
            return np.random.choice(
                n_available, min(self.n_per_epoch, n_available), replace=False
            )
...
class FRPINNFNOTrainer:
    def __init__(self, config=None, csv_path=None, device=None):
        ...
        self.medium_library = create_default_medium_library(
            z_min=config.get('z_min', 0.0),
            z_max=config.get('z_max', 800e3),
            nz=config.get('nz', 64),
            ...
            n_perturbed=medium_cfg.get('n_perturbed_models', 30),
            seed=medium_cfg.get('seed', 42),
            base_models=medium_cfg.get('base_models', None),
        )
        self.models_list = self.medium_library.models

        self.layer_info_dict = {}
        self.medium_profiles_tensor = {}
        for i, model in enumerate(self.models_list):
            self.layer_info_dict[i] = model.layer_info
            for freq_idx, freq in enumerate(self.frequencies):
                profile = model.discretize(freq=freq)
                key = (i, freq_idx)
                self.medium_profiles_tensor[key] = (
                    torch.tensor(profile, dtype=torch.float32, device=self.device)
                )

        self.model = FRPINNFNO(
        ...
        self.loss_fn = FRPINNsFNOLoss(
            layer_info_dict=self.layer_info_dict,
            ...
            num_medium_models=len(self.models_list),
        ).to(self.device)

        self.dataset = MultiMediumDataset(csv_path=csv_path)

        self.samplers = {}
        for i, model in enumerate(self.models_list):
            self.samplers[i] = PhysicsSampler(
        ...
        self.eval_callback = EvaluationCallback(
            ...
            save_dir=config.get('output', {}).get('eval_dir', 'evaluation_fno'),
        )
        self.logger = TrainingLogger(
            log_dir=config.get('output', {}).get('log_dir', 'logs'),
            log_file='frpinn_fno_training.log',
        )
        self.medium_scheduler = MediumScheduler(
            n_models=len(self.models_list),
            strategy=training_cfg.get('medium_strategy', 'random'),
            n_per_epoch=training_cfg.get('n_medium_per_epoch_phase1', 5),
        )

    def _build_batch_for_medium(self, medium_idx, freq_idx):
        sampler = self.samplers[medium_idx]
        sampling_cfg = self.config.get('sampling', {})
        batch = {}

        batch['interior'] = sampler.sample_interior(
            sampling_cfg.get('n_interior', 3000), freq_idx, self.device
        )
        batch['gauge'] = sampler.sample_gauge(
            sampling_cfg.get('n_gauge', 1500), freq_idx, self.device
        )
        batch['interface'] = sampler.sample_interface(
            sampling_cfg.get('n_interface', 500), freq_idx, self.device
        )
        batch['radiation'] = sampler.sample_radiation(
            sampling_cfg.get('n_radiation', 300), freq_idx, self.device
        )
        batch['symmetry'] = sampler.sample_symmetry(
            sampling_cfg.get('n_sym', 500), freq_idx, self.device
        )

        if len(self.dataset) > 0:
            mask = self.dataset.medium_indices == medium_idx
            if mask.any():
                indices = torch.where(mask)[0]
                n_avail = len(indices)
                n_select = min(sampling_cfg.get('n_data', 800), n_avail)
                sel = indices[torch.randperm(n_avail)[:n_select]]
                batch['data_x'] = self.dataset.data_x[sel].to(self.device)
                batch['data_y'] = self.dataset.data_y[sel].to(self.device)
            else:
                batch['data_x'] = None
                batch['data_y'] = None
        else:
    ...
    def _get_differential_lr_optimizer(self, lr=0.001, fno_lr_scale=0.1,
                                       weight_decay=1e-4):
        param_groups = self.model.get_parameter_groups(
            fno_lr_scale=fno_lr_scale, base_lr=lr
        )
        loss_params = list(self.loss_fn.parameters())
        param_groups.append({
            'params': loss_params, 'lr': lr, 'name': 'loss_fn'
        })
    ...
    def train_phase1(self, n_epochs=8000, lr=0.001, warmup_epochs=1000,
                     resample_interval=10, n_medium_per_epoch=5, grad_clip=1.0,
                     scheduler_type='cosine_warmup',
                     early_stopping_patience=0, checkpoint_interval=1000,
                     use_differential_lr=False, fno_lr_scale=0.1):
        print(f"Phase 1: AdamW training for {n_epochs} epochs")
        print(f"Device: {self.device}")

        if use_differential_lr:
            optimizer = self._get_differential_lr_optimizer(
                lr=lr, fno_lr_scale=fno_lr_scale
            )
        else:
        ...
        self.medium_scheduler.n_per_epoch = n_medium_per_epoch
        ...
        for epoch in range(n_epochs):
            ...
            medium_indices = self.medium_scheduler.get_medium_indices(
                epoch, n_epochs
            )
            ...
            for mid_idx in medium_indices:
                for freq_idx in range(n_freqs):
                    cache_key = (mid_idx, freq_idx)
                    if epoch % resample_interval == 0 or cache_key not in batch_cache:
                        batch_cache[cache_key] = self._build_batch_for_medium(
                            mid_idx, freq_idx
                        )
                    batch = batch_cache[cache_key]

                    medium_profile = self.medium_profiles_tensor[(mid_idx, freq_idx)]
                    layer_info = self.layer_info_dict[mid_idx]

                    optimizer.zero_grad()
                    total_loss, loss_dict = self.loss_fn(
                        self.model, batch, medium_profile,
                        layer_info, freq_idx,
                        medium_idx=mid_idx, epoch=epoch
                    )
                    total_loss.backward()
            ...
            if not isinstance(scheduler, ReduceLROnPlateau):
                ...
                scheduler.step(total_loss_epoch / max(1, n_batches))
            ...
            if checkpoint_interval > 0 and (epoch + 1) % checkpoint_interval == 0:
                state = {
                    ...
                    'config': self.config,
                    'n_medium_models': len(self.models_list),
                }
            ...
            eval_result = self.eval_callback.evaluate(
                ...
                medium_profiles=self.medium_profiles_tensor
            )
    ...
    def train_phase2(self, n_epochs=2000, lr=1.0, max_iter=20,
                     resample_interval=10, n_medium_per_epoch=3,
                     early_stopping_patience=0, checkpoint_interval=500):
        ...
        for epoch in range(n_epochs):
            ...
            medium_indices = np.random.choice(
                n_models, min(n_medium_per_epoch, n_models), replace=False
            )
            ...
            for mid_idx in medium_indices:
                for freq_idx in range(n_freqs):
                    batch = self._build_batch_for_medium(mid_idx, freq_idx)
                    medium_profile = self.medium_profiles_tensor[(mid_idx, freq_idx)]
                    layer_info = self.layer_info_dict[mid_idx]

                    closure_loss_dict = {}

                    def closure(b=batch, mp=medium_profile, li=layer_info,
                                fi=freq_idx, mi=mid_idx, ep=self.current_epoch,
                                cld=closure_loss_dict):
                        optimizer.zero_grad()
                        loss, ld = self.loss_fn(
                            self.model, b, mp, li, fi,
                            medium_idx=mi, epoch=ep
                        )
                        loss.backward()
                        cld.update(ld)
                        return loss
    ...
    def train(self, phase1_epochs=8000, phase2_epochs=2000,
              ...
              n_medium_per_epoch_phase1=5, n_medium_per_epoch_phase2=3,
        ...
        self.train_phase1(n_epochs=phase1_epochs, lr=lr,
                          ...
                          n_medium_per_epoch=n_medium_per_epoch_phase1,
                          grad_clip=grad_clip,
                          scheduler_type=scheduler_type,
                          early_stopping_patience=early_stopping_patience,
                          checkpoint_interval=checkpoint_interval,
                          use_differential_lr=use_differential_lr,
                          fno_lr_scale=fno_lr_scale)
        self.train_phase2(n_epochs=phase2_epochs,
                          resample_interval=resample_interval,
                          n_medium_per_epoch=n_medium_per_epoch_phase2,
                          early_stopping_patience=early_stopping_patience,
                          checkpoint_interval=checkpoint_interval)

    def predict(self, x, medium_idx, freq_idx):
        self.model.eval()
        with torch.no_grad():
            x = x.to(self.device)
    ...
    def predict_new_medium(self, x, medium_profile, freq):
        self.model.eval()
        with torch.no_grad():
            x = x.to(self.device)
            if isinstance(medium_profile, np.ndarray):
                medium_profile = torch.tensor(
                    medium_profile, dtype=torch.float32, device=self.device
                )
            pred = self.model(x, medium_profile)
        return pred

    def predict_on_grid(self, medium_idx, freq_idx, n_rho=100, n_z=100,
                        source_pos=None):
        self.model.eval()
        rho_vals = np.linspace(self.rho_range[0], self.rho_range[1], n_rho)
        z_vals = np.linspace(self.z_range[0], self.z_range[1], n_z)
        ...
        with torch.no_grad():
            pred = self.model(x_tensor, medium_profile)

        pred_grid = pred.cpu().numpy().reshape(n_z, n_rho, -1)
        return rho_grid, z_grid, pred_grid

    def evaluate_all_media(self, freq_idx=0, n_points=2000):
        self.model.eval()
        results = {}
        n_models = len(self.models_list)

        for mid_idx in range(n_models):
            sampler = self.samplers[mid_idx]
            ...
            pred_local = self.model.forward_local_only(x)

            field_norm = pred.norm(p=2).item()
            diff_norm = (pred - pred_local).norm(p=2).item()
            fno_contribution = diff_norm / (field_norm + 1e-30)

            results[mid_idx] = {
                'field_norm': field_norm,
                'local_norm': pred_local.norm(p=2).item(),
                'fno_contribution_ratio': fno_contribution,
    ...
    def compute_film_analysis(self, medium_idx=0, freq_idx=0):
        self.model.eval()
        medium_profile = self.medium_profiles_tensor[(medium_idx, freq_idx)]
        if medium_profile.dim() == 2:
            medium_profile = medium_profile.unsqueeze(0)

        gamma_norms, beta_norms = self.model.get_film_parameters(medium_profile)
        attention_weights = self.model.get_attention_weights(medium_profile)
        spectral_features = self.model.get_spectral_features(medium_profile)

        analysis = {
            'gamma_norms': gamma_norms,
            'beta_norms': beta_norms,
            'gamma_mean': float(np.mean(gamma_norms)),
            'beta_mean': float(np.mean(beta_norms)),
            'attention_weights': (attention_weights.cpu().numpy()
                                  if attention_weights is not None else None),
            'n_spectral_layers': len(spectral_features),
        }
    ...
    def plot_loss_history(self, save_dir=None):
        ...
        fig.suptitle('FRPINNs-FNO Training Loss Convergence', fontsize=14, fontweight='bold')
        plt.tight_layout()

        save_path = os.path.join(save_dir, 'frpinn_fno_loss_convergence.png')
        plt.savefig(save_path, dpi=150, bbox_inches='tight')
        plt.close()
    ...
    def plot_lr_schedule(self, save_dir=None):
        ...
        ax.set_title('Learning Rate Schedule (FRPINNs-FNO)')
        ax.grid(True, alpha=0.3)
        save_path = os.path.join(save_dir, 'frpinn_fno_lr_schedule.png')
        plt.savefig(save_path, dpi=150, bbox_inches='tight')
        plt.close()
    ...
    def plot_gradient_norms(self, save_dir=None):
        if save_dir is None:
            save_dir = 'checkpoints'
        os.makedirs(save_dir, exist_ok=True)
        save_path = os.path.join(save_dir, 'frpinn_fno_gradient_norms.png')
    ...
    def plot_eval_history(self, save_dir=None):
        ...
        fig.suptitle('FRPINNs-FNO Evaluation Metrics', fontsize=14, fontweight='bold')
        plt.tight_layout()

        save_path = os.path.join(save_dir, 'frpinn_fno_eval_metrics.png')
        plt.savefig(save_path, dpi=150, bbox_inches='tight')
        plt.close()

    def plot_film_analysis(self, medium_idx=0, freq_idx=0, save_dir=None):
        if save_dir is None:
            save_dir = 'checkpoints'
        os.makedirs(save_dir, exist_ok=True)

        analysis = self.compute_film_analysis(medium_idx, freq_idx)

        fig, axes = plt.subplots(1, 3, figsize=(18, 5))

        axes[0].bar(range(len(analysis['gamma_norms'])), analysis['gamma_norms'],
                     color='#3498db', alpha=0.8)
        axes[0].set_xlabel('Layer')
        axes[0].set_ylabel('Norm')
        axes[0].set_title('FiLM Gamma Norms per Layer')
        axes[0].grid(True, alpha=0.3)

        axes[1].bar(range(len(analysis['beta_norms'])), analysis['beta_norms'],
                     color='#e74c3c', alpha=0.8)
        axes[1].set_xlabel('Layer')
        axes[1].set_ylabel('Norm')
        axes[1].set_title('FiLM Beta Norms per Layer')
        axes[1].grid(True, alpha=0.3)

        if analysis['attention_weights'] is not None:
            attn = analysis['attention_weights'].flatten()
            axes[2].plot(attn, linewidth=1.0, color='#2ecc71')
            axes[2].set_xlabel('Depth Index')
            axes[2].set_ylabel('Attention Weight')
            axes[2].set_title('FNO Attention Weights')
            axes[2].grid(True, alpha=0.3)
        else:
            axes[2].text(0.5, 0.5, 'No attention weights available',
                         ha='center', va='center', transform=axes[2].transAxes)

        fig.suptitle(f'FiLM Analysis (Medium {medium_idx}, Freq {freq_idx})',
                     fontsize=14, fontweight='bold')
        plt.tight_layout()

        save_path = os.path.join(save_dir, 'frpinn_fno_film_analysis.png')
        plt.savefig(save_path, dpi=150, bbox_inches='tight')
        plt.close()
    ...
    def generate_training_report(self, save_dir=None):
        ...
        report_path = os.path.join(save_dir, 'training_report_fno.txt')
        with open(report_path, 'w', encoding='utf-8') as f:
            f.write("=" * 60 + "\n")
            f.write("FRPINNs-FNO Training Report\n")
            f.write("=" * 60 + "\n\n")

            f.write(f"Device: {self.device}\n")
            f.write(f"Model parameters: {sum(p.numel() for p in self.model.parameters()):,}\n")
            f.write(f"FNO encoder parameters: "
                    f"{sum(p.numel() for p in self.model.fno_encoder.parameters()):,}\n")
            f.write(f"Number of medium models: {len(self.models_list)}\n")
            ...
            f.write("\nMedium Models:\n")
            for i, model in enumerate(self.models_list):
                f.write(f"  Model {i}: {model.name} ({len(model.layers)} layers)\n")
    ...
    def export_predictions(self, output_dir, medium_idx=0, freq_idx=0,
                           n_rho=100, n_z=100):
        os.makedirs(output_dir, exist_ok=True)
        rho_grid, z_grid, pred_grid = self.predict_on_grid(
            medium_idx, freq_idx, n_rho, n_z
        )

        np.savez(
            os.path.join(output_dir, f'predictions_mid{medium_idx}_freq{freq_idx}.npz'),
            rho_grid=rho_grid,
            z_grid=z_grid,
        ...
        with open(os.path.join(output_dir,
                               f'prediction_summary_mid{medium_idx}_freq{freq_idx}.json'), 'w') as f:
...
def main():
    parser = argparse.ArgumentParser(description='FRPINNs-FNO Training - Chapter 4')
    ...
    if args.config is not None:
        ...
        config = config_to_frpinn_fno_args(raw_config)
    else:
        default_config_path = os.path.join(project_root, 'config_frpinn_fno.yaml')
    ...
    if csv_path is None:
        csv_path = os.path.join(project_root, 'data', 'lmgf_fno_data.csv')

    if not os.path.exists(csv_path):
        print(f"Warning: CSV data file not found at {csv_path}")
        print("Creating placeholder CSV file for FNO multi-medium data...")
        ...
        with open(csv_path, 'w', newline='') as f:
            ...
            header = (['x', 'y', 'z', 'xp', 'yp', 'zp', 'rho', 'f_log',
                       ...
                       'Kyz_Re', 'Kyz_Im', 'Kzx_Re', 'Kzx_Im',
                       'Kzy_Re', 'Kzy_Im', 'Kzz_Re', 'Kzz_Im',
                       'Kphi_Re', 'Kphi_Im', 'medium_idx'])
            writer.writerow(header)
            writer.writerow([0.0] * 22 + [0])
        print(f"Placeholder CSV created at {csv_path}")
        print("Please replace with actual multi-medium LMGF data.")

    trainer = FRPINNFNOTrainer(config=config, csv_path=csv_path, device=args.device)
    ...
    print("=" * 60)
    print("FRPINNs-FNO Training - Chapter 4")
    print("=" * 60)
    print(f"Frequencies: {config['frequencies']}")
    print(f"Number of medium models: {len(trainer.models_list)}")
    print(f"Device: {trainer.device}")
    print(f"Model parameters: {sum(p.numel() for p in trainer.model.parameters()):,}")
    print(f"FNO encoder parameters: "
          f"{sum(p.numel() for p in trainer.model.fno_encoder.parameters()):,}")
    print("=" * 60)
    ...
    trainer.train(
        ...
        use_differential_lr=training_cfg.get('use_differential_lr', False),
        fno_lr_scale=training_cfg.get('fno_lr_scale', 0.1),
    )

    output_cfg = config.get('output', {})
    save_dir = os.path.join(project_root, output_cfg.get('checkpoint_dir', 'checkpoints'))
    trainer.save_model(os.path.join(save_dir, 'frpinn_fno_model.pt'))
    trainer.save_loss_history(os.path.join(save_dir, 'frpinn_fno_loss_history.json'))
    ...
    trainer.plot_eval_history(save_dir=save_dir)
    trainer.plot_film_analysis(save_dir=save_dir)
