...
class NTKAdaptiveWeights(nn.Module):
    def __init__(self, num_losses=5, update_interval=100, eps=1e-8,
    ...
    def compute_ntk_trace(self, model, x, output_indices=None):
...
class GradientBasedResidualWeighting(nn.Module):
    def __init__(self, num_components=7, eps=1e-8, momentum=0.9):
        super().__init__()
        self.num_components = num_components
        self.eps = eps
        self.momentum = momentum
        self.register_buffer('component_weights',
                             torch.ones(num_components, dtype=torch.float32) / num_components)
        self.register_buffer('gradient_norms',
                             torch.ones(num_components, dtype=torch.float32))
        self._initialized = False

    def update(self, residuals, model_params):
        grad_norms = []
        for res in residuals:
            if res.requires_grad:
                grads = torch.autograd.grad(
                    res.mean(), model_params,
                    create_graph=False, retain_graph=True,
                    allow_unused=True
                )
                total_norm = 0.0
                ...
                grad_norms.append(math.sqrt(total_norm + self.eps))
            else:
                grad_norms.append(1.0)
        norms_tensor = torch.tensor(grad_norms, device=self.component_weights.device)
        if not self._initialized:
            self.gradient_norms.copy_(norms_tensor)
            self._initialized = True
        else:
            self.gradient_norms.mul_(self.momentum).add_(
                (1.0 - self.momentum) * norms_tensor
            )
        inv_norms = 1.0 / (self.gradient_norms + self.eps)
        self.component_weights.copy_(inv_norms / (inv_norms.sum() + self.eps))

    def get_weights(self):
        with torch.no_grad():
            return self.component_weights.tolist()


class CausalTrainingWeighting(nn.Module):
    def __init__(self, num_chunks=10, eps=1e-8, temperature=1.0):
        super().__init__()
        self.num_chunks = num_chunks
        self.eps = eps
        self.temperature = temperature
        self.register_buffer('causal_weights',
                             torch.ones(num_chunks, dtype=torch.float32))
...
class LossAnnealingScheduler:
    def __init__(self, total_epochs, warmup_epochs=1000,
                 pde_start=0.1, pde_end=1.0,
                 data_start=1.0, data_end=0.5,
                 gauge_start=0.5, gauge_end=1.0,
                 bc_start=0.5, bc_end=1.0,
                 sym_start=0.3, sym_end=1.0):
        self.total_epochs = total_epochs
        self.warmup_epochs = warmup_epochs
        self.schedules = {
            'pde': (pde_start, pde_end),
            'data': (data_start, data_end),
            'gauge': (gauge_start, gauge_end),
            'bc': (bc_start, bc_end),
            'sym': (sym_start, sym_end),
        }

    def get_multipliers(self, epoch):
        if epoch < self.warmup_epochs:
            progress = epoch / self.warmup_epochs
        else:
            progress = 1.0
        multipliers = {}
        for name, (start, end) in self.schedules.items():
            current = start + (end - start) * progress
            multipliers[name] = current
        return multipliers


class PerComponentLossTracker:
...
class LossDiagnostics:
    ...
    def record(self, loss_dict, model=None):
    ...
    def record_gradient_norms(self, loss_dict, model):
        for name in self.loss_names:
            if name in loss_dict:
                loss_val = loss_dict[name]
                if isinstance(loss_val, torch.Tensor) and loss_val.requires_grad:
                    grads = torch.autograd.grad(
                        loss_val, model.parameters(),
                        retain_graph=True, allow_unused=True
                    )
                    total_norm = 0.0
                    for g in grads:
                        if g is not None:
                            total_norm += (g.detach() ** 2).sum().item()
                    self.gradient_norms[name].append(math.sqrt(total_norm))
...
class FRPINNsLoss(nn.Module):
    def __init__(self, layer_info, frequencies, eps=1e-8,
                 beta1=1.0, beta2=1.0, adaptive=True, rho_range=None,
                 use_complex_km=False, weighting_strategy='ema',
                 ntk_update_interval=100, causal_chunks=10,
                 causal_temperature=1.0, annealing_total_epochs=10000,
                 annealing_warmup=1000, track_per_component=True):
        super().__init__()
        ...
        self.rho_range = rho_range or (0.1, 50.0)
        ...
        if self.adaptive:
            if weighting_strategy == 'ntk':
                self.weight_module = NTKAdaptiveWeights(
                    num_losses=5, update_interval=ntk_update_interval
                )
            else:
                self.weight_module = AdaptiveLossWeights(num_losses=5)

        self.residual_weighting = GradientBasedResidualWeighting(
            num_components=7
        )
        self.causal_weighting = CausalTrainingWeighting(
            num_chunks=causal_chunks, temperature=causal_temperature
        )
        self.annealing_scheduler = LossAnnealingScheduler(
            total_epochs=annealing_total_epochs,
            warmup_epochs=annealing_warmup
        )
        self.component_tracker = PerComponentLossTracker()
        self.diagnostics = LossDiagnostics()
    ...
    def _compute_gradient(self, Kc, x_in):
        grad = torch.autograd.grad(Kc.sum(), x_in, create_graph=True)[0]
        return grad
    ...
    def compute_pde_loss(self, model, x_interior, layer_info, freq_idx,
                         use_causal=False, use_residual_weighting=False):
        x = x_interior[:, 0:1]
        y = x_interior[:, 1:2]
        z = x_interior[:, 2:3]
        xp = x_interior[:, 3:4]
        ...
        if use_residual_weighting:
            residuals_list = []
            for Kc_Re, Kc_Im in zip(s0_components_Re, s0_components_Im):
                lap_Re = self._compute_laplacian_cylindrical(Kc_Re, x_in, rho)
                res_Re = lap_Re + km2 * Kc_Re
                lap_Im = self._compute_laplacian_cylindrical(Kc_Im, x_in, rho)
                res_Im = lap_Im + km2 * Kc_Im
                residuals_list.extend([res_Re.mean(), res_Im.mean()])
            self.residual_weighting.update(residuals_list, list(model.parameters()))

        return loss_pde, component_losses

    def compute_gauge_loss(self, model, x_gauge, layer_info, freq_idx):
    ...
    def compute_bc_loss(self, model, x_interface, x_radiation,
        ...
        if x_interface is not None and x_interface.shape[0] > 0:
            ...
            pred_above = model(x_above)
            pred_below = model(x_below)
        ...
        if x_radiation is not None and x_radiation.shape[0] > 0:
            ...
            pred_rad_grad = model(x_rad_grad)
    ...
    def compute_data_loss(self, model, x_data, y_data, freq_idx):
        pred = model(x_data)
    ...
    def compute_symmetry_loss(self, model, x_sym, num_rot_samples=1):
        ...
        if num_rot_samples > 0:
            ...
            rot_components_Re = [pred[:, 0:1], pred[:, 10:11], pred[:, 12:13]]
            rot_components_Im = [pred[:, 1:2], pred[:, 11:12], pred[:, 13:14]]
            rot_pred_Re = [pred_rot[:, 0:1], pred_rot[:, 10:11], pred_rot[:, 12:13]]
            rot_pred_Im = [pred_rot[:, 1:2], pred_rot[:, 11:12], pred_rot[:, 13:14]]

            for orig_Re, rot_Re in zip(rot_components_Re, rot_pred_Re):
                loss_rot = loss_rot + ((orig_Re - rot_Re) ** 2).mean()
            for orig_Im, rot_Im in zip(rot_components_Im, rot_pred_Im):
                loss_rot = loss_rot + ((orig_Im - rot_Im) ** 2).mean()
    ...
    def compute_multi_freq_loss(self, model, batches, layer_info,
                                freq_indices=None):
        ...
        for freq_idx in freq_indices:
            batch = batches.get(freq_idx, None)
            if batch is None:
                continue

            loss_pde, pde_comp = self.compute_pde_loss(
                model, batch['interior'], layer_info, freq_idx
            )
            loss_gauge, gauge_comp = self.compute_gauge_loss(
                model, batch['gauge'], layer_info, freq_idx
            )
            ...
            if batch.get('data_x') is not None and batch.get('data_y') is not None:
                loss_data, data_comp = self.compute_data_loss(
                    model, batch['data_x'], batch['data_y'], freq_idx
                )
            ...
            if batch.get('symmetry') is not None:
                loss_sym, sym_comp = self.compute_symmetry_loss(
                    model, batch['symmetry']
                )

            freq_loss = loss_pde + loss_gauge + loss_bc + loss_data + loss_sym
            total_loss = total_loss + freq_loss

            for k, v in pde_comp.items():
                all_component_losses[f'f{freq_idx}_{k}'] = v
        ...
        total_loss = total_loss / n_freq
        return total_loss, all_component_losses

    def forward(self, model, batch, freq_idx, epoch=0):
        ...
        use_residual_weighting = epoch > 1000 and epoch % 200 == 0

        loss_pde, pde_comp = self.compute_pde_loss(
            model, x_interior, self.layer_info, freq_idx,
            use_causal=use_causal,
            use_residual_weighting=use_residual_weighting
        )
        loss_gauge, gauge_comp = self.compute_gauge_loss(
            model, x_gauge, self.layer_info, freq_idx
        )
        loss_bc, bc_comp = self.compute_bc_loss(
            model, x_interface, x_radiation, self.layer_info, freq_idx
        )
        ...
        if x_data is not None and y_data is not None:
            loss_data, data_comp = self.compute_data_loss(
                model, x_data, y_data, freq_idx
            )

        loss_sym = torch.tensor(0.0, device=loss_pde.device)
        sym_comp = OrderedDict()
        if x_sym is not None:
            loss_sym, sym_comp = self.compute_symmetry_loss(model, x_sym)
        ...
        annealing_multipliers = self.annealing_scheduler.get_multipliers(epoch)
        annealed_total = total_loss
        for name, mult in annealing_multipliers.items():
            pass
        ...
        if self.track_per_component:
            self.component_tracker.update(loss_dict)

        self.diagnostics.record(loss_dict)

        if self.weighting_strategy == 'ntk' and self.adaptive:
            loss_inputs = [x_interior, x_gauge, x_interface, x_data, x_sym]
            self.weight_module.update_weights(model, loss_inputs)
